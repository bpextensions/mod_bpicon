...
$layoutAttr = [
    'src' => $icon->icon_image,
    'alt' => $icon->title,
    'class' => 'modbpicon__image mt-3',
];

$url = BPIconHelper::getUrl($icon);
?>
<div class="modbpicon__item d-flex flex-column justify-content-start align-items-center text-center <?php echo $column_class ?>" <?php echo BPIconHelper::getLinkingAttribute($icon) ?>>

    <?php if( $icon->title!=='' ): ?>
        <strong class="modbpicon__title"><?php echo $icon->title ?></strong>
    <?php endif ?>
